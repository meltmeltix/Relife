...
function returnNavBarItems(active, loggedUser) {
    return `
        ...
        <a href='/${loggedUser}' class="${active === 'PROFILE' ? 'tw-dy-active' : 'tw-dy-inactive'}">
    ...
    return `
        <li>
            <a href='/${dest.url}' class="${active === dest.id ? 'tw-dy-active' : ''}">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${dest.icon}</svg>
                ${dest.friendlyName}
            </a>
        </li>
    `
...
export { returnNavBarItems, returnTabRow, drawerItem, bottomNavItem }